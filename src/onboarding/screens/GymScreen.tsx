...
import {
  ...
  ErrorBanner, ProgressBar, BackBtn, Spinner,
} from "../ui";
import { supabase } from "@/integrations/supabase/client";

// gyms table uses is_public (not is_private)
interface GymResult {
  id: string;
  name: string;
  city: string | null;
...
export function GymScreen() {
  const { goTo, setGym } = useOnboarding();
  const [query, setQuery] = useState("");
  const [results, setResults] = useState<GymResult[]>([]);
  const [searching, setSearching] = useState(false);
  const [searchError, setSearchError] = useState<string | null>(null);
  const [selected, setSelected] = useState<GymResult | null>(null);
  ...
  function handleJoinRequest() {
    // TODO: wire to gym service joinRequest RPC
    setJoinRequestSent(true);
    if (selected) {
      setGym({ gymId: selected.id, gymName: selected.name, isPrivate: !selected.is_public, joinRequestSent: true });
    }
  }
  ...
  return (
    <Screen>
      ...
      <div className="flex flex-col gap-6 flex-1">
        ...
        {searchError && <ErrorBanner message={searchError} onRetry={() => search(query)} />}
        ...
        {selected && (
          <div className="panel border-primary/40 bg-primary/5 p-4 flex flex-col gap-3" role="status">
        ...
        {!searching && query.length >= 2 && results.length === 0 && !selected && (
          <div className="panel p-5 text-center flex flex-col gap-2" role="status">
            ...
            <p className="text-xs text-muted-foreground">Try a different name, or skip to join later.</p>
          </div>
        )}
        ...
        <div className="mt-auto flex flex-col gap-3 pt-4">
          <PrimaryBtn
            ...
            disabled={isPrivateSelected && !joinRequestSent}
          >
            {selected ? "Confirm gym" : "Skip for now"}
          </PrimaryBtn>
          {isPrivateSelected && !joinRequestSent && (
            <p className="text-xs text-center text-muted-foreground">
              Send a join request to continue with this gym.
            </p>
          )}
        </div>
      </div>
